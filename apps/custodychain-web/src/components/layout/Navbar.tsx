...
export default function Navbar() {
  ...
  const { ledgerId } = useWeb3();
  const fetchPageUrl = `/ledger/${ledgerId}/fetch`;
  const createPageUrl = `/ledger/${ledgerId}/create`;
  const myEvidencesUrl = `/ledger/${ledgerId}/my-evidences`;
  ...
  const isEvidenceActive =
    pathname === `/ledger/${ledgerId}/my-evidences` ||
    pathname.startsWith(`/ledger/${ledgerId}/evidence/`);
